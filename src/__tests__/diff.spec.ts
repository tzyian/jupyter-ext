...
describe('buildDiffSegments', () => {
  test('returns unchanged segments when identical', () => {
    ...
    expect(segs[0]).toMatchObject({ value: 'line1', type: 'unchanged', lineNumberOriginal: 1, lineNumberNew: 1 });
    expect(segs[1]).toMatchObject({ value: 'line2', type: 'unchanged', lineNumberOriginal: 2, lineNumberNew: 2 });
  });
  ...
  test('handles added and removed lines', () => {
    ...
    expect(types).toEqual(expect.arrayContaining(['unchanged', 'removed', 'added']));
  });
});
